...
async function seedTopics(userIds: { [key: string]: string }) {
  const client = await pool.connect();
...
async function seedMessages(topicIds: string[], userIds: { [key: string]: string }) {
  const client = await pool.connect();
  ...
  try {
    ...
    for (let i = 0; i < demoMessages.length && i < userEmails.length; i++) {
      ...
      const existingMessage = await client.query(
        'SELECT id FROM messages WHERE topic_id = $1 AND user_id = $2 AND content = $3 LIMIT 1',
        [topicId, userId, demoMessages[i].content]
      );
